...
var cityPageTem = '<section id="cityListWraper" class="city_list_wraper page">'+
						...
						'<div id="cityBox" onscroll="topStop(this)" class="speedup">'+
							...
							'<div class="city_list">'+
								...
								'<div class="city_item" onclick="cityListView.clickLoctionCity()"><span id="locCity"><#=locCityName#></span></div>'+
							'</div>'+
...
var cinemaListView = (function(){
	//影院列表页面模板
	var cinemaPageTem = '<div id="cinameListWrapper" class="cimane_list_wraper">'+
							'<div style="display:none;" class="header_nav_box">'+
								'<div class="header_nav">'+
									'<div class="back"><a href="javascript:window.history.back(-1);"><span class="back_img icon24 i400"></span></a></div>'+
									'<span class="title fzl">选择影院</span>'+
									'<div class="oper fzs" onclick="cinemaListView.showCityList()"><span>1234</span><span id="cityName" class="fzs"></span></div>'+
								'</div>'+
							'</div>'+
						...
						'</div>';
	//影院列表数据模板
	var cinemaListTem = '<ul class="film_local">'+
							'<#for(var i=0;i<data.length;i++){ #>'+
								'<#var cinemaInfo = data[i]; #>'+
								'<li class="cinema_info" onclick="cinemaListView.chooseCinema(\'<#=cinemaInfo.cinema.cid#>\',\'<#=cinemaInfo.cinema.name#>\',\'<#=cinemaInfo.address#>\')" >'+
									'<p><#=cinemaInfo.cinema.name#></p>'+
									'<p><#=cinemaInfo.address#></p>'+
									'<img src="<#=basePath#>images/cinema_link.png" class="oper_arrow" />'+
								'</li>'+
							'<#}#>'+
						'</ul>';
	var opts={};
	...
	function init(options,bathPath){
		...
		$("#cinameListWrapper").remove();
		document.body.innerHTML+=cinemaPageHtml;
		historyRec.pushView({'viewId':'cinameListWrapper'},"","");
...
//	        url:_bathPath+"/info/cinema.do",
	...
	function chooseCinema(cinemaId,cinemaName,cinemaAdd){
		...
		eval(opts.callback).call(this,cinemaId,cinemaName,cinemaAdd);
		document.getElementById("cinameListWrapper").style.display="none";
	}
	//点击影院列表展现城市列表方法
	function showCityList(){
		var cityOpts = {'hideEle':['cinameListWrapper'],
...
function showCinemaListBySl(){
 ...
 function getFilmShowByCinema(cinemaId,cinemaName,cinemaAdd){
    	window.history.back(-1);
    	$(".cinema_name").text(cinemaName);
        $(".cinema_address").text(cinemaAdd);
